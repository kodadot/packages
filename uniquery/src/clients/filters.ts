...
export function getFilters (filters: FilterBuilder[], provider: Provider) {
  const mappingFn = isSubQuery(provider) ? subqueryFilterMapping : subsquidFilterMapping
  return generateFilters(filters, mappingFn)
}

function generateFilters (filters: FilterBuilder[], mappingFn: FilterMappingFn): string[] {
  return filters.map(([type, directions]) => (
...
function subsquidFilterMapping (filter: FilterType, direction: FilterOrderDirection): string {
  return appendFilterDirection(filter, direction)
}

function appendFilterDirection (filter: string, direction: FilterOrderDirection) {
  return filter + '_' + direction
}

function getFilterOrders (filterOrders: FilterOrderType): FilterOrderDirection[] {
  if (!filterOrders) {
    return ['ASC', 'DESC']
  }
